...
# Extended Particle Filter Function Student-t Distribution
#------------------------------------------------------------
...
# Extended Particle Filter Function Normal Distribution
#------------------------------------------------------------
...
# Joint Likelihood Function Normal Distribution
#------------------------------------------------------------
...
# Joint Likelihood Function Student-t Distribution
#------------------------------------------------------------
...
# Metropolis Hasting Sampler for Extended Particle Filter Student-t Distribution
#------------------------------------------------------------
def Metropolis_Hasting_Sampler_T(y, x1, x2, z, true_theta):
    ...
    R_matrix = R_matrix * 0.0000008 # gives 0.29
...
# Metropolis Hasting Sampler for Extended Particle Filter Normal Distribution
#------------------------------------------------------------
def Metropolis_Hasting_Sampler_N(y, x1, x2, z, true_theta):
    ...
    # Scale the matrix according to the acceptance rate
    R_matrix = R_matrix * 0.000001 #0.00000008 gives 0.175
    ...
    sum_absolute_percent_deviation = 0
    # Start MCMC
    for count in range(total_draws):
        theta_star = theta_g + multivariate_normal.rvs(mean=np.zeros(parameter_count), cov=R_matrix)

        # Find the percent deviation from theta_g to theta_star
        #percent_deviation = (theta_star - theta_g)*100 / theta_g
        ...
        #if (count+1) % 100 == 0:
            # Calculate and print the rolling average of the percent deviation
            #rolling_average = np.around(sum_absolute_percent_deviation / (count + 1), 3)
            #print(f'Rolling average of absolute percent deviation: {rolling_average}')
            # print the count
            #print("Count:", (count+1))
...
def test_MH_sampler():
    ...
    sample_std_N = np.empty((iterations,), dtype=object)
    sample_std_T = np.empty((iterations,), dtype=object)
    ...
    while i < iterations:
        ...
        if indicator_T == True and indicator_N == True:
            ...
            sample_std_N[i] = np.std(samples_N, axis=1)
            sample_std_T[i] = np.std(samples_T, axis=1)
    ...
    mean_sample_std_N = np.mean(sample_std_N, axis=0)
    mean_sample_std_T = np.mean(sample_std_T, axis=0)
    ...
    return sample_mean_N, sample_mean_T, mean_sample_std_N, mean_sample_std_T, acceptance_mean_N, acceptance_mean_T, mean_MSE_T, mean_MSE_N
