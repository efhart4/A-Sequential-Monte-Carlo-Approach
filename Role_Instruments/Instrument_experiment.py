...
# Extended Particle Filter Function WO instruments
#------------------------------------------------------------
def Extended_Particle_Filter_WO(y, x1, x2, theta_in):
    """
    ...
    for k in range(1, Sample):
        ...
        # Find the likelihood of the proposed states using a joint likelihood that represents the likelihood of the measurement error and the endogenous variable
        likelihood_over_particles = joint_likelihood(error_vec, x1[k], mu, Sigma)
...
# Extended Particle Filter Function W instruments
#------------------------------------------------------------
def Extended_Particle_Filter_W(y, x1, x2, z,  theta_in):
    """
    Estimate the path of a time varying parameter using a particle filter.
...
# Prior Probability for Extended Particle Filter WO instruments
#------------------------------------------------------------
def prior_probability_WO(theta_in, uniform_priors):
    """
    ...
    prior_product = ( prob_beta * prob_TVP0 * prob_measurement_StdDev * prob_TVP_StdDev 
                     * prob_correlation_coefficient * prob_instrumental_StdDev)
...
# Prior Probability for Extended Particle Filter W instruments
#------------------------------------------------------------
def prior_probability_W(theta_in, uniform_priors):
    """
    Calculate the log prior probability of the model parameters.
...
# Metropolis Hasting Sampler for Extended Particle Filter WO instruments
#------------------------------------------------------------
def Metropolis_Hasting_Sampler_WO(y, x1, x2, true_theta):
    """
    ...
    R_matrix[4, 4] = 1 # Instrument_StdDev
    R_matrix[5, 5] = 1 # correlation_coefficient


    # Scale the matrix according to the acceptance rate
    R_matrix = R_matrix * 0.0000003# 0.000001 gives 0.22 #0.0000025 gives 0.17 #  0.00001 give .05


    # Initial parameter values for the sampler. All comparisons are made using the true value as the starting value.
    theta_g = true_theta[:6]
    
    # find the log prior 
    prior_indicator_g, log_prior_g = prior_probability_WO(theta_g, uniform_priors)

    
    # calculate the likelihood of the data conditoned on the parameters using the extended particle filter
    pf_indicator_g, path_g, pY_theta_g = Extended_Particle_Filter_WO(y, x1, x2, theta_g)
    ...
    for count in range(total_draws):
        ...
        # find the log prior 
        prior_indicator_star, log_prior_star = prior_probability_WO(theta_star, uniform_priors)
        ...
        if prior_indicator_star == True:
            ...
            pf_indicator_star, path_star, pY_theta_star = Extended_Particle_Filter_WO(y, x1, x2, theta_star)
...
# Metropolis Hasting Sampler for Extended Particle Filter W instruments
#------------------------------------------------------------
def Metropolis_Hasting_Sampler_W(y, x1, x2, z, true_theta):
    """
    sample model parameters using Metropolis Hasting Sampler
    ...
    # Scale the matrix according to the acceptance rate
    R_matrix = R_matrix * 0.0000008 # 0.000001  gives .30 #0.0000025 gives 0.17 #  0.00001 give .05
    ...
    # find the log prior 
    prior_indicator_g, log_prior_g = prior_probability_W(theta_g, uniform_priors)

    
    # calculate the likelihood of the data conditoned on the parameters using the extended particle filter
    pf_indicator_g, path_g, pY_theta_g = Extended_Particle_Filter_W(y, x1, x2, z, theta_g)
    ...
    for count in range(total_draws):
        ...
        # find the log prior 
        prior_indicator_star, log_prior_star = prior_probability_W(theta_star, uniform_priors)
        
        # check if the prior is finite
        if prior_indicator_star == True:

            # calculate the likelihood of the data conditoned on the parameters using the extended particle filter if the prior is finite
            pf_indicator_star, path_star, pY_theta_star = Extended_Particle_Filter_W(y, x1, x2, z, theta_star)
...
def test_MH_sampler():
    ...
    while i < iterations:
        ...
        indicator_WO, sample_paths_WO, samples_WO, uniform_priors_WO, acceptance_WO =  Metropolis_Hasting_Sampler_WO(y, x1, x2, theta)

        # MH sampler with instruments
        indicator_W, sample_paths_W, samples_W, uniform_priors_W, acceptance_W  =  Metropolis_Hasting_Sampler_W(y, x1, x2, z, theta)
        ...
        if indicator_W == True and indicator_WO == True:

            # print data summary while testing
            print("\n")         
            print("Particle Filter W")
            print("Indicator:", indicator_W)
            print("Acceptance Rate:", acceptance_W)
            print("Median of samples:", np.round(np.median(samples_W, axis=1), decimals=3) )   

            # print data summary while testing
            print("\n")         
            print("Particle Filter WO Indicator:", indicator_WO)
            ...
            samples_MH_W[i] = np.median(samples_W, axis=1)
            acceptance_MH_W[i] = acceptance_W
    ...
    sample_mean_WO =  np.mean(samples_MH_WO, axis=0) 
    sample_mean_W = np.mean(samples_MH_W, axis=0) 
